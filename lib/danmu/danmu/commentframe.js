module.exports = {
	init: function(DD) {
		 ...
		 */
		DD.CommentFrame.prototype.addSprite = function(text, style, color, font, lifeTime) {
		...
		DD.CommentFrame.prototype.addCustomSprite = function(clazz, param) {
			eval(clazz);
			param = JSON.parse(param); //这部分直接转为JSON进行使用。
		 ...
		 * 停止显示弹幕，停止创建弹幕
		 */
		DD.CommentFrame.prototype.stopDanmu = function() {
			this.clearDanmu();
		 ...
		 * 重新打开弹幕功能
		 */
		DD.CommentFrame.prototype.restartDanmu = function() {
			this.danmuState = true;
		};
		/**
		 * @Override
