 ...
 */
Player.prototype.init = function (elementId, url, wsUrl) {
	this.setup(elementId, url, wsUrl);
};

/**
 * 初始化方法
 */
Player.prototype.setup = function (elementId) {
	// 初始化绘制canvas相关的组件
...
Player.prototype.addDanmu = function () {
	...
	this.danmus.forEach(function (info) {
		if (info.style == "Custom") {
			that.frame.addCustomSprite(info.className, info.param);
		} else {
