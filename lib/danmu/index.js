...
// 监听得到弹幕的事件
global.coordinator.on("gotDanmu", function(data) {
    player.parseDanmus(data);
    player.controlDanmu("update");
});
// 监听操作相关事件
global.coordinator.on("danmuControl", function(data) {
    window.danmuControl[data].call();
});
// 删除弹幕事件
global.coordinator.on("deleteDanmu", function(data) {
...
function init(config, listener, object) {
    ...
    window.setInterval(function() {
        global.coordinator.emit("fps", player.frame.fps);
    }, 1000);
